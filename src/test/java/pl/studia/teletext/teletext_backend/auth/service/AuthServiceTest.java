...
public class AuthServiceTest {
  ...
  void shouldThrowRefreshTokenExpiredExceptionWhenRefreshTokenIsExpired() {
    ...
    when(refreshTokenService.refreshToken(anyString()))
        .thenThrow(new RefreshTokenExpiredException("Token expired"));

    // when & then
    assertThrows(
        RefreshTokenExpiredException.class, () -> authService.reauthenticate(oldRefreshToken));
  }
  ...
  void shouldThrowRefreshTokenNotFoundExceptionWhenRefreshTokenIsNotFound() {
    ...
    when(refreshTokenService.refreshToken(anyString()))
        .thenThrow(new RefreshTokenNotFoundException("Token not found"));

    // when & then
    assertThrows(
        RefreshTokenNotFoundException.class, () -> authService.reauthenticate(oldRefreshToken));
  }
  ...
  void shouldThrowUsernameNotFoundExceptionWhenUserIsNotPresent() {
    ...
    // when & then
    assertThrows(
        UsernameNotFoundException.class, () -> authService.reauthenticate(oldRefreshToken));
  }
