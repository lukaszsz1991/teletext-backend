...
public class TvpClient {
  private final static String BASE_PREFIX = "prasa";
  ...
  private String buildXmlFileName(String channelCode, LocalDate date) {
    var fileName = String.format("m%4d%02d%02d_%s.xml",
      date.getYear(),
      date.getMonthValue(),
      date.getDayOfMonth(),
      channelCode);
    log.debug("Built TVP XML file name: {}. File built with code {} and date {}.", fileName, channelCode, date);
    return fileName;
  }
}
