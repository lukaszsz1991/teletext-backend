  ...
  public void validate(Map<String, Object> config) {
    ...
    for(String field : requiredFields()) {
      if(!config.containsKey(field)) {
        throw new InvalidJsonConfigException("Brak wymaganego pola w konfiguracji sport-matches: " + field);
      }
    }
    ...
    if(config.get("league") instanceof String category) {
      try {
        ...
        throw new InvalidJsonConfigException("Pole league musi być wartością enum FootballLeague w konfiguracji sport-matches");
      }
    } else {
      throw new InvalidJsonConfigException("Pole league musi być ciągiem znaków w konfiguracji sport-matches");
    }

    if(!(config.get("week") instanceof Integer)) {
      throw new InvalidJsonConfigException("Pole week musi być liczbą w konfiguracji sport-matches");
    }
  }
