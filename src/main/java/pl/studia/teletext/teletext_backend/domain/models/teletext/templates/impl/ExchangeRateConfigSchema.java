  ...
  public void validate(Map<String, Object> config) {
    ...
    for(String field : requiredFields()) {
      if(!config.containsKey(field)) {
        throw new InvalidJsonConfigException("Brak wymaganego pola w konfiguracji exchange-rate: " + field);
      }
    }

    if(!(config.get("currencyCode") instanceof String)) {
      throw new InvalidJsonConfigException("Pole currencyCode musi być ciągiem znaków w konfiguracji exchange-rate");
    }

    if(!(config.get("lastCount") instanceof Integer)) {
      throw new InvalidJsonConfigException("Pole lastCount musi być liczbą w konfiguracji exchange-rate");
    }
  }
